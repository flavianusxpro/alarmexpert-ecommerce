        ...
        <Container>
            <div className="md:gap-10 gap-4 grid grid-cols-1 sm:grid-cols-2 w-full">
                {articles.map((article) => (
                    ...
                    >
                        <h1 className="text-[26px] font-semibold mb-3 text-[#0A3353]">{article.description}</h1>
                        <Button className="bg-white py-8 px-25 w-[99px] font-bold text-[#0A3353] text-[20px]" radius="full">
                            {article.buttonText}
                        </Button>
                    </div>
                ))}
